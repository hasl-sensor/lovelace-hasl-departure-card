...
export class HASLDepartureCard extends LitElement implements LovelaceCard {
    ...
    getLayoutOptions() {
        ...
        // TODO: better calculate the size of the card
        let itemsSize = 0;
        const entity = this.config?.entity;
        const data = this.hass?.states[entity]
        if (isDepartureAttrs(data.attributes)) {
            const itemsLength = data.attributes.departures.length - 2;
            itemsSize = Math.max(0, Math.min(
                itemsLength,
                this.config?.max_departures || itemsLength
            ));
        }

        const size = [
            // !!this.config.title ? 1 : 0,
            // (!!this.config.entity && !!this.config.show_entity_name) ? 1: 0,
            // !!this.config.show_header ? 1 : 0,
            itemsSize,
        ].reduce((sum, entity) => sum += entity ? entity : 0, 1);
    ...
    render() {
        ...
        const departures = this.config?.show_departures ? this._getDepartures() : nothing ;
    ...
    private _getDepartures = () => {
        const entity = this.config?.entity;
        const data = this.hass?.states[entity]

        if (entity === undefined) return nothing;
        if (data === undefined) return nothing;
        if (!isDepartureAttrs(data.attributes)) return nothing;
        ...
        const config: PartialEntityConfig = {
            ...
            maxDepartures: this.config?.max_departures || attrs.departures.length,
        }

        return html`<hasl4-departure-entity
            .hass=${this.hass}
            .config=${config}
            .departures=${attrs.departures}
